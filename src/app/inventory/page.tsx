...
const emptyInbound = {
  ...
  sku: "", name: "", category: "", unit: "个", salePrice: "", reorderLevel: "0",
};
...
export default function InventoryPage() {
  ...
  const [maintenance, setMaintenance] = useState({ name: "", category: "", unit: "个", sale: "", reorder: "0" });
  ...
  const openItem = async (item: Item, tab: DetailTab = "maintenance") => {
    ...
    setCountCost("");
    setError("");
    setMaintenance({ name: item.name, category: item.category ?? "", unit: item.unit, sale: (item.default_sale_price_cents / 100).toFixed(2), reorder: String(item.reorder_level) });
    try {
  ...
  const saveMaintenance = async () => {
    ...
    try {
      const updated = await json<RawItem>(`/api/inventory/${selected.id}`, {
        ...
        body: JSON.stringify({ name: maintenance.name, category: maintenance.category || null, unit: maintenance.unit, default_sale_price_cents: Math.round((Number(maintenance.sale) || 0) * 100), reorder_level: Math.max(0, Number(maintenance.reorder) || 0) }),
      });
  ...
  const typeLabel = (type: string) => ({ PURCHASE_RECEIPT: "采购入库", CASE_RESERVE: "维修预留", CASE_RELEASE: "释放预留", CASE_CONSUMPTION: "维修消耗", COUNT_ADJUSTMENT: "盘点修正", MANUAL_ADJUSTMENT: "其他入库" })[type] ?? type;
    ...
    <div className="page-shell space-y-6">
      ...
      <div className="surface-panel overflow-x-auto">
        <table className="data-table w-full min-w-[1050px] text-sm">
          ...
          <tbody>
            {visibleItems.map((item) => <tr key={item.id} className="border-b last:border-0"><td className="p-4"><p className="font-semibold">{item.name}</p><p className="font-mono text-xs text-muted-foreground">{item.sku} · {item.unit}</p></td><td className="p-4">{item.category ?? "-"}</td><td className="p-4 text-right font-medium">{item.on_hand_qty}</td><td className="p-4 text-right text-amber-700 dark:text-amber-300">{item.reserved_qty}</td><td className="p-4 text-right font-bold">{item.available_qty}</td><td className="p-4 text-right">{formatCents(item.avg_cost_cents)}</td><td className="p-4 text-right">{formatCents(item.inventory_value_cents)}</td><td className="p-4">{!item.is_active ? <span className="text-muted-foreground">停用</span> : item.is_low_stock ? <span className="text-red-600 dark:text-red-400">低库存</span> : <span className="text-emerald-700 dark:text-emerald-300">正常</span>}</td><td className="p-4"><Button variant="outline" onClick={() => openItem(item)}><Settings2 className="mr-2 size-4" />维护 / 盘点</Button></td></tr>)}
